...
|--------|-------------|-------|
| **Tween** | Animate a single value from A → B with easing | [tween.md](tween.md) |
...
1. **Clock** produces `dt` (seconds since last frame)
2. **AnimationDriver** calls `update(dt)` on every active animation
3. Each animation (**Tween**, **KeyframeTrack**, **Spring**) advances its internal state
...
Update            — update(&mut self, dt: f32) -> bool
    └── Implemented by: Tween, KeyframeTrack, Timeline, Spring
```
...
├── timeline.rs      — Timeline, Sequence, TimelineState
...
- **Leptos/Dioxus** — WASM driver patterns
